...
def ddx(f):
  ...
  if is_x(f):
    ...
    if op == "+":
      ...
      result = left + " * " + ddx(right) + " + " + right + " * " + ddx(left)
    elif op == "/":
      result = "(" + ddx(left) + " * " + right + " - " + left + " * " + ddx(right) + ") / (" + right + "^2" 
    elif op == "^":
      if has_x(left) and is_number(right):   # x ^3    x = left   3 = right 
        result = right + " * " + left + "^" + str(int(right)-1) + " * " + ddx(left)
      else:
...
def taylor(f, N, A):
  ...
  for n in range(N+1):
      if n == 0:
        terims = f"({current_f.replace('x',A)})"
      else:
        current_f = ddx(current_f)
        terims = f"({current_f.replace('x',A)}) * (x-{A})^{n}"
      
      terims.append(f"({terims}) / {factorial(n)}")
...
def try_function(name, f, x, A):
  ...
  print(f, "≈", approx)
  print(f.replace("x", x + " - " + A), "≈", eval(approx.replace("^", "**").replace("x", x)))
  
  plt.clf()
  R = [x * 0.1 for x in range(100)]
  plt.plot(R, [eval(approx.replace("^", "**").replace("x", str(x))) for x in R], color = 'C0')
  plt.plot(R, [eval(f.replace("^", "**").replace("x", str(x))) for x in R], color = 'C1')
