...
int main() {
    ...
    // Hata hesaplaması ve hata toleransının sağlanması
    ...
    // Hata toleransının sağlanması için döngü
    while (hata > tolerans) {
        // Isı transferi simülasyonunun bir adım daha ileri götürülmesi
        sonuçlar = ısıTransferiSimülasyonu(alpha, delta_t, delta_x, adım_sayısı);
    ...
    // Hata toleransı sağlandığında hata toleransını ve son sıcaklık değerlerini yazdır
    cout << "Hata toleransı sağlandı: " << hata << " <= " << tolerans << endl;

    // Hata toleransı sağlandığında son sıcaklık değerlerini yazdırma
    cout << "Son sıcaklık değerleri:" << endl;
